...
"""ESC/P byte stream for the Nepal Gas Udyog continuous-form invoice (LQ-310).

Clone of escp_invoice.py (Nepal Gas Invoice Dot Matrix) with its own entry point
so its coordinates can be calibrated independently of the original Nepal Gas
form. Everything below is inherited from that module; adjust POS here only.
...
Y0_MM = -7.7  # origin unchanged from Nepal Gas per user; the -2mm "pull up" is
              # applied to each element's y in POS below instead of here.
              # measured 2026-07-14 with the centre-circle target: the printer
...
POS = {
	"copy_label":   (118.5, 34.7),  # x = CENTRE (label is centred at emit time)
	"invoice_no":   (35.0, 27.5),  # starts 3.5cm from left
	"ref_inv":      (74.0, 43.7),
	"trans_date":   (193.0, 28.0),  # printed at 12cpi so the full date fits before the 215.4mm head limit
	"invoice_date": (193.0, 33.3),  # same column/pitch as trans date, 5.3mm below
	"do_no":        (193.0, 38.5),
	"customer":     (52.0, 44.0),  # 5.2cm from left
	"address":      (52.0, 49.0),
	"pan":          (52.0, 55.0),
	"body_top":     (0, 72.0),  # first item row
	"row_h":        4.8,
	"words":        (48.0, 87.1),  # wraps at 13.5cm (51 chars at 15cpi), continues down
