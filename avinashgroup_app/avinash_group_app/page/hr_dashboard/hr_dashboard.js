// HR Dashboard — one BS month of HR at a glance, with a grouped side menu.
...
frappe.pages["hr-dashboard"].on_page_load = function (wrapper) {
	...
	wrapper.hr_dashboard = new HRDashboard(page);
};
...
class HRDashboard {
	constructor(page) {
		...
		this.$root = $(`<div class="hrd">
			<aside class="hrd-nav" aria-label="${__("HR menu")}"></aside>
			...
			<div class="hrd-nav-body">
				...
				${this.groups
					.map(
						...
						<ul>${g.items
							.map(
								(i) => `<li><a href="${href(i)}" data-search="${esc((i.label + " " + i.name).toLowerCase())}">
	...
	kpis(d) {
		...
		const tiles = [
			...
			{ tone: "amber", ic: "clock", label: __("Late arrivals"), value: num(a.late),
				...
				route: ["List", "Attendance"], opts: { late_entry: 1, docstatus: 1 } },
	...
	attendance_card(d) {
		...
		const body = a.marked
			? `<div class="hrd-legend">
				...
				<div class="hrd-chart">${this.attendance_svg(a.days)}<div class="hrd-tip" role="status"></div></div>`
			...
			.map((day, i) => {
				...
				const rects = segs
					.map(([k, cls], j) => {
						...
						return `<path class="${cls}" d="${roundTop(x, y1, barw, h, isTop ? 3 : 0)}"/>`;
					})
					.join("");
				const weekend = day.weekday === "Sat" ? `<rect class="off" x="${L + i * bw}" y="${T}" width="${bw}" height="${plot_h}"/>` : "";
				const label = day.bs_day === 1 || day.bs_day % 5 === 0 ? `<text x="${x + barw / 2}" y="${H - 8}" class="tick" text-anchor="middle">${day.bs_day}</text>` : "";
				return `<g class="day" data-i="${i}">${weekend}${rects}${label}
					<rect class="hit" x="${L + i * bw}" y="${T}" width="${bw}" height="${plot_h}"/></g>`;
			})
			.join("");
		return `<svg viewBox="0 0 ${W} ${H}" role="img" aria-label="${__("Daily attendance")}">${grid}${bars}</svg>`;
	}
