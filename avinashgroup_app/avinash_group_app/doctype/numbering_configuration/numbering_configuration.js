...
function set_smart_value_options(frm, cdt, cdn) {
	...
	const apply = (fieldtype, options) => {
		...
		if (!value_df) return;
		// options is a string for Select/Link, an array for MultiSelect
		if (
			value_df.fieldtype === fieldtype &&
			JSON.stringify(value_df.options || "") === JSON.stringify(options || "")
		) return;
		value_df.fieldtype = fieldtype;
	...
	// Is Set / Is Not Set compare against nothing — the value is ignored.
	if (!row.field || op === "Is Set" || op === "Is Not Set") {
		apply("Data", "");
		return;
	}
