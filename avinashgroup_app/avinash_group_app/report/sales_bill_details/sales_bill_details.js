...
var sbd_fy = erpnext.utils.get_fiscal_year(frappe.datetime.get_today(), false, true)
	? erpnext.utils.get_fiscal_year(frappe.datetime.get_today(), true)
	: [];

frappe.query_reports["Sales Bill Details"] = {
	filters: [
		{
			fieldname: "company",
			label: __("Company"),
		...
		},
		{
			...
			label: __("From Date"),
			fieldtype: "Date",
			default: sbd_fy[1] || frappe.datetime.month_start(),
			reqd: 1,
		},
		{
			fieldname: "to_date",
			label: __("To Date"),
			fieldtype: "Date",
			default: sbd_fy[2] || frappe.datetime.now_date(),
			reqd: 1,
		},
		{
	...
	onload: function (_report) {
		...
		const sbd_selected_columns = function (print_settings) {
		...
		_report.page.add_inner_button(__('Download PDF'), function () {
			...
			const url = '/api/method/avinashgroup_app.avinash_group_app.report.sales_bill_details.sales_bill_details.download_pdf'
		...
		_report.print_report = function (print_settings) {
			...
			const url = '/api/method/avinashgroup_app.avinash_group_app.report.sales_bill_details.sales_bill_details.download_pdf'
				...
				+ '&selected_columns=' + encodeURIComponent(JSON.stringify(sbd_selected_columns(print_settings)))
		...
		if (!frappe.ui.get_print_settings._pick_columns_patched) {
			...
			const _our_reports = ['Sales Register Report', 'Purchase Register Report', 'Sales Bill Details'];
