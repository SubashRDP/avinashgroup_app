...
frappe.ui.form.on("Purchase Invoice", {
    ...
    items_add: function(frm, cdt, cdn) {
        ...
        // ALWAYS set default TDS Apply On to Percentage (%) for new rows
...
frappe.ui.form.on("Purchase Invoice Item", {
    item_code: async function(frm, cdt, cdn) {
        ...
        if (row && row.item_code) {
            try {
                ...
                // Always set TDS Apply On to Percentage (%) by default FIRST
                ...
                if (item_data.message) {
                    ...
                    // Set TDS Rate (since we're in Percentage mode by default)
    ...
    qty: function(frm, cdt, cdn) {
        ...
        // In Percentage mode, backend will recalculate VAT and TDS on save
        frm.refresh_field('items');
    },
    ...
    base_net_amount: function(frm, cdt, cdn) {
        ...
        // Backend will recalculate custom_total, VAT and TDS
        frm.refresh_field('items');
    },
    ...
    base_net_rate: function(frm, cdt, cdn) {
        ...
        // Backend will recalculate base_net_amount, custom_total, VAT and TDS
        frm.refresh_field('items');
    },
    ...
    custom_tds_apply_on: async function(frm, cdt, cdn) {
        ...
        if (row.custom_tds_apply_on === "Amount") {
            // Clear rate when switching to Amount mode
            frappe.model.set_value(cdt, cdn, "custom_tds_rate", 0);
        }
    ...
    custom_tds_rate: function(frm, cdt, cdn) {
        let row = locals[cdt][cdn];
        if (row && row.custom_tds_apply_on === 'Percentage (%)') {
            console.log(`TDS rate changed for ${row.item_code}: ${row.custom_tds_rate}%`);
            // Backend will recalculate TDS amount on save
        }
        frm.refresh_field('items');
    },
    ...
    custom_tds_amount: function(frm, cdt, cdn) {
        let row = locals[cdt][cdn];
        if (row && row.custom_tds_apply_on === 'Amount') {
    ...
    apply_tds: function(frm, cdt, cdn) {
        ...
        // If unchecked, clear TDS amount and rate
        if (!row.apply_tds) {
...
function toggle_tds_fields(frm, cdt, cdn) {
    if (!frm.fields_dict.items || !frm.fields_dict.items.grid) {
        frappe.after_ajax(() => toggle_tds_fields(frm, cdt, cdn));
        return;
    }
    ...
    if (row.custom_tds_apply_on === "Percentage (%)") {
        ...
        console.log(`TDS Percentage mode for ${row.item_code}: Rate readonly (${row.custom_tds_rate || 0}%), Amount hidden`);
    }
    ...
    if (row.custom_tds_apply_on === "Amount") {
        ...
        console.log(`TDS Amount mode for ${row.item_code}: Rate hidden, Amount editable`);
    }
}
...
async function populate_tds_rate_from_custom_category(frm) {
    ...
    try {
        ...
        if (category_data.message && category_data.message.rates && category_data.message.rates.length > 0) {
            ...
            // Set TDS rate in all items where apply_tds is checked AND in Percentage mode
            if (frm.doc.items && frm.doc.items.length > 0) {
                for (let i = 0; i < frm.doc.items.length; i++) {
                    let item = frm.doc.items[i];
                    
                    // Only set rate if apply_tds is checked AND in Percentage mode
                    if (item.apply_tds && item.custom_tds_apply_on === 'Percentage (%)') {
                                    ...
                                    return frappe.db.get_doc('Account', account.name)
                                        .then(account_doc => {
                                            if (account_doc.custom_sub_type_list && account_doc.custom_sub_type_list.length > 0) {
                                                // Changed to vehicle_list with filter
                                                account_subtype_cache[account.name] = account_doc.custom_sub_type_list.map(
                                                    item => item.vehicle_list
                                                ).filter(vehicle => vehicle); // Filter out null/undefined
                                            } else {
    ...
    return new Promise((resolve, reject) => {
        ...
        frappe.db.get_doc('Account', account)
            .then(account_doc => {
                ...
                if (account_doc.custom_sub_type_list && account_doc.custom_sub_type_list.length > 0) {
                    // Changed to vehicle_list with filter
                    vehicles = account_doc.custom_sub_type_list.map(
                        item => item.vehicle_list
                    ).filter(vehicle => vehicle); // Filter out null/undefined
                }
                
                // Store in cache
                account_subtype_cache[account] = vehicles;
                resolve(vehicles);
            })
            .catch(err => {
