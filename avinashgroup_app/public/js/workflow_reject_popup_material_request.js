(function () {
	...
	function is_target_doc(frm) {
		...
		return doctype === "Material Request";
	}

	function is_custom_approver(frm) {
		const approver = (frm && frm.doc && frm.doc.custom_material_request_approver)
			? frm.doc.custom_material_request_approver.toString().trim()
			: "";
	...
	function patch_workflow_actions() {
		...
		if (current && current.__patched_reject_material_request) {
			return true;
		}
		...
		frappe.ui.form.States.prototype.show_actions = function () {
			if (!this.frm || !is_target_doc(this.frm)) {
							...
							const run_workflow = () => {
								...
								const persist_reject_reason = () => {
									if (is_reject_action(d.action, d) && is_target_doc(frm) && frm.__reject_reason) {
										return frappe.xcall("avinashgroup_app.custom_code.workflow_material_request.set_reject_reason", {
											doctype: frm.doctype,
											name: frm.doc.name,
		...
		frappe.ui.form.States.prototype.show_actions.__patched_reject_material_request = true;
		return true;
	}
