# Document Numbering — Admin & User Guide

How the **Document No.** and **Voucher No.** on transactions are generated, and how
to configure it from the desk with **Numbering Configuration** rules.
...
- The number shown **before saving is a preview**. The real, collision-free number
  is locked **on Save**, so two people never get the same one.
...
### Level 1 — Built-in defaults (no configuration)
...
With no rule, the **scope** (what makes the count restart) is **Company + Type-code + Fiscal Year**.
...
you control **when** it fills (conditions) and **what scope** it counts by (segments).

**Precedence for the number:**
1. **User typed a value** → always kept (manual override).
2. **A matching Auto-fill rule** → the rule decides (its Document No. conditions can
   also *restrict* numbering — it is authoritative).
...
the previous one with no separator (e.g. number `104` + word `A` → `104A`).
...
a receipt with no branch is named but **not** auto-numbered.
...
## 9. Manual override, hints & errors
...
Run the automated suite (45 tests, incl. randomized fuzz + concurrency):
...
Inspect why a document gets its number, from the console:
...
d.payment_type = "Receive"; d.custom_p_type = "Bank Customers Receipt"
d.custom_branch = "…"

ns._match_numbering_rule(d)       # which rule wins
ns._docno_scope(d)                # the scope/series it resolves to (or None)
